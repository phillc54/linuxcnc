    ...
    PmVector const * const end)
{
    ...
    if (start) line9->start = *start;
    if (end) line9->end = *end;

    line9->uVec = VecVecSub(line9->end, &line9->start);
    line9->tmag = VecMag(&line9->uVec);
    VecScalMultEq(&line9->uVec, 1./line9->tmag);
    return 0;
}
...
{
    PmVector no_abc = VecVecSub(line9->end, &line9->start);
...
int pmLine9Point(const PmLine9 * const line9, double progress, PmVector * const point)
{
    ...
    *point = VecScalMult(line9->uVec, progress);
    return VecVecAddEq(point, &line9->start);
}
...
int pmLine9Cut(PmLine9 * const line9, double cut_pt_progress, SegmentToKeepType keep_pt)
{
    ...
    case KEEP_END_PT:
    {
        PmVector tmp = VecScalMult(line9->uVec, cut_pt_progress);
        VecVecAddEq(&line9->start, &tmp);
        line9->tmag -= cut_pt_progress;
        return 0;
    }
    case KEEP_START_PT:
        line9->end = VecScalMult(line9->uVec, cut_pt_progress);
