...
PmVector VecCopyFrom(const PmVector * const from)
{
    PmVector v;
...
{
    VecVecAddEq(&v1, v2);
    // NOTE: v1 + NULL is considered to be v1 + nothing = v1
    return v1;
}
...
{
    VecVecSubEq(&v1, v2);
    // Note: if v2 is NULL then the result is v1 - nothing = v1
    return v1;
}
    ...
    PmVector * const v)
{
    ...
    *v = VecScalMult(*u, -dot);
    ...
    if (0 != VecUnitEq(v)) {
        // Failed to create a unit vector (v is too short), which means a and b are very close to parallel.
        // Therefore, the orthonormal bases that contains both vectors is just a single vector u.
        // This isn't an error per se but the caller should know about it.
        return 1;
    }
    return 0;
}
                        ...
                        PmVector const * const v)
{
    int i;
    for (i = 0; i < PM_VECTOR_SIZE; ++i) {
        u.ax[i] = pmSq(u.ax[i]) + pmSq(v->ax[i]);
    }
    return u;
}
