...
PM_CARTESIAN::PM_CARTESIAN(PM_CONST PM_CYLINDRICAL PM_REF c)
{
    ...
    toCyl(c, &cyl);
    pmCylCartConvert(&cyl, (PmCartesian*)this);
}
...
PM_CARTESIAN::PM_CARTESIAN(PM_CONST PM_SPHERICAL PM_REF s)
{
    ...
    toSph(s, &sph);
    pmSphCartConvert(&sph, (PmCartesian*)this);
}
...
// Clunky but maybe faster way to tes
bool PM_CARTESIAN::is_tiny(double threshold) const
{
...
PM_CIRCLE::PM_CIRCLE(const PM_CARTESIAN &start, const PM_CARTESIAN &end, PM_CARTESIAN const &center, PM_CARTESIAN const &normal, int turn, double expected_angle_rad)
{
    init(start, end, center, normal, turn, expected_angle_rad);
}
#endif

int PM_CIRCLE::init(PM_CARTESIAN const &start, PM_CARTESIAN const &end,
    PM_CARTESIAN const &_center, PM_CARTESIAN const &_normal, int turn, double expected_angle_rad)
{
    return pmCircleInit(this, &start, &end, &_center, &_normal, turn, expected_angle_rad);
}

int PM_CIRCLE::point(double angle_in, PM_CARTESIAN &point) const
{
    return pmCirclePoint(this, angle_in, &point);
}
...
PM_CARTESIAN proj(PM_CARTESIAN const &v1, PM_CARTESIAN const &v2)
{
    PM_CARTESIAN ret;
    pmCartCartProj(&v1, &v2, &ret);
    return ret;
}
...
PM_CARTESIAN shift(const PM_CARTESIAN & p, int n)
{
...
void rotate_xy(PM_CARTESIAN & v, double theta_rad) {
    const double x_in = v.x, y_in = v.y;
...
double abs_angle_xy(const PM_CARTESIAN & v)
{
