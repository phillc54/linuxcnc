...
struct PM_CARTESIAN : public PmCartesian {
    ...
    void shift(int n);
    bool is_tiny(double threshold=DOUBLE_FUZZ) const;
};
...
#endif
    PM_CIRCLE(const PM_CARTESIAN &start, const PM_CARTESIAN &end, PM_CARTESIAN const &center, PM_CARTESIAN const &normal, int turn, double expected_angle_rad);

    /* functions */
    int init(PM_CARTESIAN const &start, PM_CARTESIAN const &end,
    PM_CARTESIAN const &center, PM_CARTESIAN const &normal, int turn, double expected_angle_rad);
    int point(double angle_, PM_CARTESIAN &point) const;
};
...
extern void rotate_xy(PM_CARTESIAN &v, double theta_rad);
extern double abs_angle_xy(PM_CARTESIAN const &v);
