...
// KLUDGE copy-paste helper functions from TP
static inline double sq(double x) {return x*x;}

static inline double hypot2(double x, double y)
{
    return pmSqrt(x*x + y*y);
}

static inline double hypot4(double w, double x, double y, double z)
{
    return pmSqrt(w*w + x*x + y*y + z*z);
}
    ...
    double expected_angle_rad)
{
    ...
    pmCartLineInit(&circ9->abc, &start_abc, &end_abc);
    pmCartLineInit(&circ9->uvw, &start_uvw, &end_uvw);
 ...
 * nominal speed.
 */
PmCircle9Err findSpiralArcLengthFit(
    PmCircle const * const circle,
    ...
    double angle_tolerance)
{
    ...
    if (res_angle != 0) {
        return PM_CIRCLE9_SPIRAL_FIT_FAILED;
    }

    // Ensure that the fit has the same total angle within a reasonable tolerance
    double fit_err = angle_end_chk - circle->angle;
    if (fabs(fit_err) > angle_tolerance) {
        return PM_CIRCLE9_SPIRAL_FIT_ANGLE_DIFF;
    }

    return 0;
}

double spiralEffectiveRadius(double start_radius, double angle, double spiral)
{
    double dr = spiral / angle;

    // Exact representation of spiral arc length flattened into
    double n_inner = sq(dr) + sq(start_radius);
...
double pmCircleEffectiveMinRadius(PmCircle const * circle)
{
    ...
    double r_spiral = spiralEffectiveRadius(circle->radius, circle->angle, circle->spiral);
