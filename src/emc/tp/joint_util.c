...
unsigned jointAccelViolation(int joint_idx, double acc)
{
    ...
    if (a_max_nominal > 0) {
        emcmotStatus->current_accel_norm = fmax(emcmotStatus->current_accel_norm, fabs(acc / a_max_nominal));
    }
...
unsigned jointMaxPositionViolation(int joint_idx, double position)
{
    ...
    return (unsigned)(position > emcmotDebug->joints[joint_idx].max_pos_limit + ABS_TOL) << joint_idx;
}

unsigned jointMinPositionViolation(int joint_idx, double position)
{
    static const double ABS_TOL = 1e-6;
    return (unsigned)(position < emcmotDebug->joints[joint_idx].min_pos_limit - ABS_TOL) << joint_idx;
}
...
unsigned findAccelViolations(PmVector axis_accel)
{
    ...
    // Reset this before walking through to find the worst case
    emcmotStatus->current_accel_norm = 0;
...
unsigned findMinPositionViolations(PmVector axis_pos)
{
    ...
    unsigned fail_bits = (unsigned)(0x0);
    for (int i=0; i < PM_VECTOR_SIZE; ++i) {
        fail_bits |= jointMinPositionViolation(i, axis_pos.ax[i]);
    }
    return fail_bits;
}
