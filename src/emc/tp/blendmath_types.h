...
typedef struct {
    ...
    double path_tolerance;   /* Net blend tolerance (min of line 1 and 2) */
    double req_min_radius;   /* Explicitly request a minimum radius (not always satisfied if other contraints are hit) */
    double correction_dist;  /* The requested minimum path "deviation" required for the blend to pass through the nominal endpoint */
...
typedef struct {
    ...
    double arc_intersection_halfangle; /* Intersection angle, half of angle between -u1 and u2*/
    double a_n_max;     /* max normal acceleration allowed */
    double a_max_planar;       /* max acceleration allowed for blend */

    double R_goal;      // Overall solver goal factoring in various limits
...
typedef struct {
    ...
    double acc;
    EndConditionType end_condition;
} EndCondition;
...
typedef struct
{
    ...
    PmVector u2;
    PmVector P;
    double s1; //!< target position (in path units) where blend starts
    double s2; //!< target position (in path units) where blend ends
    double prev_len; //!< length of previous segment used for blend (ideally, target - s1)
} blend_boundary_t;
...
typedef struct
{
    ...
    PmVector u_mid;
    PmVector P_mid;
    double d1; //!< Length of first blend's intermediate segment
    double d2; //!< Length of second blend's intermediate segment
} biarc_control_points_t;


typedef enum
{
    BIARC_MIN_LENGTH_LIMITED=-4,
    BIARC_DEGENERATE_SEGMENTS=-3,
    BIARC_FAILED_TO_CONVERGE=-2,
    BIARC_EXCEEDED_MAX_ITERATIONS=-1,
    BIARC_NOT_SOLVED,
    BIARC_REACHED_GOAL_SIZE,
    BIARC_REACHED_TOLERANCE,
    BIARC_CONVERGED,
} BiarcSolverStatusType;

typedef struct
{
    double deviation_margin;
    double radius_rel;
    double radius_abs;
    double v_size_limit; // Limit on velocity of prev / next segment (due to length)
...
typedef struct {
    ...
    double R_geom_est; //!< Estimated miniumum blend radius for both arcs
    double arc_len_est; //!< Estimated minimum arc length for both arcs
    ...
    BlendParameters param1;
    BlendParameters param2;
} biarc_solution_t;
...
typedef struct
{
    ...
    PmVector u_diff;
    double dot;
    int is_C0_continuous;
    int is_C1_continuous;
} ContinuityCheck;

typedef struct {
    ContinuityCheck prev_to_blend1;
    ContinuityCheck blend1_to_blend2;
    ContinuityCheck blend2_to_next;
} BiarcContinuityChecks;
