...
There are several interfaces, like AXIS.
AXIS is an interface for use with normal computer and monitor.
...
* `DISPLAY = axis` - The file name of the executable providing the user interface to use.
  Prominent valid options are (all in lower case): `axis` `qtvcp`, `qtvcp-qtplasmac`.
...
* `POSTGUI_HALFILE = example2.hal` - Execute 'example2.hal' after the GUI has created its HAL pins.
  ...
  GUIs that support postgui HAL files include AXIS. +
...
* `INCREMENT = 200`
  ...
  This setting is effective with AXIS but note that some
