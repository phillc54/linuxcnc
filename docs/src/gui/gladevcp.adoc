...
- as a tab in AXIS, this would create a third tab besides the Preview and DRO tabs which must be raised explicitly,
...
- GladeVCP panels need to be embedded in a master GUI window setup, e.g., AXIS (embedded window or as an embedded tab).
...
It assumes a running LinuxCNC environment like AXIS.
To connect to it, inspects the INI_FILE_NAME environment variable.
Gremlin displays the current NGC file - it does monitor for changes and reloads the ngc file if the file name in AXIS changes.
...
* `[widget name].set_filename("../../../configs/sim/<config>/sim.var")`
...
To make this possible, a signal is sent from the GUI to the embedded tab.
...
I get an unexpected unmap event in my handler function right after startup. What's this?::
  This is a consequence of your Glade UI file having the window1 Visible property set to True, together with re-parenting the GladeVCP window into AXIS.
